...
span = int(sys.argv[4])
dataset = sys.argv[5]
print('input file: {file}\nkmer: {kmer}\nsplit: {i}\nspan: {span}\ndataset: {dataset}'.format(file=sys.argv[1] , kmer=sys.argv[2], i=sys.argv[3], span=sys.argv[4], dataset=sys.argv[5]))
...
def protein2emb(row):
    pro = row['protein']
    fileL = glob.glob(protein_emb_dir+'*_'+pro+'_*single_repr*npy', recursive=True)
    if len(fileL)>=1:
...
# output_folder = '/new-stg/home/cong/DPI/dataset/ChIP_690/deepsea/embeddings/'
...
output_folder = '/new-stg/home/cong/DPI/dataset/'+dataset+'/deepsea/embeddings/'
