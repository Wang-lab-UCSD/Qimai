...
class BIN_Interaction_Flat(nn.Sequential):
    ...
    def __init__(self, **config):
        ...
        self.flatten_dim = 3 * (self.max_d - 2) * (self.max_p - 2)
        
        # specialized embedding with positional one
        # self.demb = Embeddings(self.input_dim_dna, self.emb_size, self.max_d, self.dropout_rate)
        # self.pemb = Embeddings(self.input_dim_protein, self.emb_size, self.max_p, self.dropout_rate)
        
        self.demb = FullyConnectedEmbed(self.input_dim_dna, self.emb_size, self.dropout_rate)
        self.pemb = FullyConnectedEmbed(self.input_dim_protein, self.emb_size, self.dropout_rate)
    ...
    def forward(self, d, p, d_mask, p_mask):
        ...
        d_encoded_layers = self.d_encoder(d_emb.float(), ex_d_mask.float()) # batch_size * max_d * embed_size
        #print(d_encoded_layers.shape)
        p_encoded_layers = self.p_encoder(p_emb.float(), ex_p_mask.float()) # batch_size * max_p * embed_size
        ...
        p_aug = torch.unsqueeze(p_encoded_layers, 1).repeat(1, self.max_d, 1, 1) # repeat along dna size
        
        i = d_aug * p_aug # interaction
        i_v = i.view(int(self.batch_size/self.gpus), -1, self.max_d, self.max_p) 
        # batch_size x embed size x max_dna_seq_len x max_protein_seq_len
        i_v = torch.sum(i_v, dim = 1) # batch_size * max_d * max_p
        #print(i_v.shape)
        i_v = torch.unsqueeze(i_v, 1) # batch_size * 1 * max_d * max_p
        #print(i_v.shape)
        
        i_v = F.dropout(i_v, p = self.dropout_rate)        
        
        #f = self.icnn2(self.icnn1(i_v))
        f = self.icnn(i_v) # batch_size * 3 * (max_d - 2) * (max_p - 2)
        ...
        f = f.view(int(self.batch_size/self.gpus), -1)  # batch_size * flat_dim
        #print(f.shape)
...
class Embeddings(nn.Module):
    """Construct the embeddings from protein, position embeddings.
    """
...
class FullyConnectedEmbed(nn.Module):
    ...
    def __init__(self, nin, nout, dropout_rate=0.5, activation=nn.ReLU()):
        ...
        self.layernorm = nn.LayerNorm(nin)
