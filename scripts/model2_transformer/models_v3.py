...
class Decoder(nn.Module):
    ...
    def __init__(self, dna_dim, hid_dim):
        super().__init__()
...
#         self.encoder1 = Encoder(self.protein_dim, self.hid_dim, self.n_layers, self.n_heads, self.pf_dim, self.dropout, self.device)
        ...
        self.decoder = Decoder(self.dna_dim, self.hid_dim)



    def forward(self, dna, dna_mask):
        # torch.tensor
...
def run_epoch(data_iter,model,optimizer,scheduler,use_cuda,mode="train",accum_iter=1):
    ...
    y_label, y_pred = [], []
    for i, (d, d_mask, label) in enumerate(data_iter):
#         print(i)
#         print(d.shape)
#         print(p.shape)
#         print(d_mask.shape)
