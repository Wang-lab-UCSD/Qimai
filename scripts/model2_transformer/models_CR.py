...
class LabelSmoothBCELoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        log_probs = torch.sigmoid(inputs)
...
class Decoder(nn.Module):
    ...
    def __init__(self, dna_dim, hid_dim, n_layers, n_heads, pf_dim, dropout, device):
        ...
        self.ft2 = nn.Linear(512, hid_dim) # max_d = 512
#         self.ft2 = nn.Linear(512, hid_dim) # max_p = 512
...
#             nn.Dropout(p=self.dropout),
    ...
    def forward(self, trg, src, trg_mask=None,cross_attn_mask=None):
        ...
        # src = [batch_size, protein len, hid_dim] # encoder output
        # trg_mask = [batch size, 1, dna sent len, dna sent len]
        # cross_attn_mask = [batch size, 1, dna sent len, protein len]        
        trg = self.ft(trg)
        # trg = [batch size, dna len, hid dim]
        ...
        weight = torch.max(attention, dim=1)[0]
        weight = torch.max(weight, dim=2)[0]
        weight = self.ft2(weight)
        # weight = [batch size, hid dim]   
...
#         label, _ = torch.max(trg, dim=1)
        ...
        # label = [batch size, hid dim]     

                
        label = weight * label
        # trg = [batch size, hid dim]     
        
        label = label.unsqueeze(1)        
        label = self.output(label)
        # label = [batch size, 1, 1]
#         print(label.shape)
...
#         loss_fct = torch.nn.BCELoss() 
        loss_fct = LabelSmoothBCELoss(smoothing=smoothing)
...
#     if mode == "test":
    ...
    return accuracy1, AUC, PRC, MCC, recall, F1, total_loss / count, y_label, y_pred        
