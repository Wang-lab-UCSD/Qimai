# adapted from test8_cv_split.py

# major change on 2025/11/06: 
# ### 1. changed the motif selection criteria: The new logic will sequentially select motifs based on your criteria ("latest_chip_seq", HOCOMOCO, JASPAR) and then use a fallback mechanism to ensure three motifs are chosen if possible.
# ### 2. improved robustness of indirect tf interactors by making sure they're analyzable with known, valid motifs
# ### 3. comp.json file is more concise without losing critical information

# major change on 2025/11/07: 
# ### 1. optional two-hop indirect motif search 
# ### 2. fixed critical bug on stale mmseqs db. This is found when I checked the sei prob for novel protein FOXP2, it didn't reference FOXP4 as expected. 
# ### This is because different runs share the same cache file. However, it should change when the known protein list or the major fasta file is updated
# ### So we implemented automatic hashing: the hash will change if the list of known proteins or source of protein sequences changes. Then the script will automatically create a new separate database directory

# minor change on 2025/11/13: 
# ### 1. added processed sample number and a summary of indirect motif hits to the simple json output file
...
PROTEIN_ALIASES = {"AP2A": "TFAP2A", "AP2C": "TFAP2C", "CJUN": "JUN", "GR": "NR3C1"}
...
fimo_path = '/new-stg/home/cong/software/meme/bin/fimo'
...
# DeepSEA model ---
DEEPSEA_MODEL_PATH = "./benchmark/deepsea/best_model_params.pkl"
DEEPSEA_PROTEIN_MAP_PATH = "./benchmark/deepsea/protein_to_id_map.json"
...
# --- SEI model ---
SEI_MODEL_PATH = "./benchmark/sei/best_model_params.pkl"
SEI_PROTEIN_MAP_PATH = "./benchmark/sei/protein_to_id_map.json"
...
STRING_TF_LIST_PATH = '/new-stg/home/cong/DPI/dataset/all_human_tfs_proteins.txt'
...
SIMILARITY_ENGINE_CONFIG = 'mmseqs'
NUM_TOP_SIMILAR_FOR_AVG_LOGITS_CONFIG = int(os.environ.get("NUM_SIMILAR_LOGITS", "30")) # want to make sure all the hits are considered
MAX_LEVENSHTEIN_DIST_CONFIG = int(os.environ.get("MAX_LEV_DIST", "250"))
MIN_MMSEQS_BLAST_BITSCORE_CONFIG = int(os.environ.get("MIN_BITSCORE", "50"))
MAX_MMSEQS_BLAST_EVALUE_CONFIG = float(os.environ.get("MAX_EVALUE", "0.001"))
FETCH_MISSING_SEQUENCES_FROM_UNIPROT_CONFIG = os.environ.get("FETCH_UNIPROT", "True").lower() == "true"
UNIPROT_ORGANISM_ID_FOR_FETCH_CONFIG = "9606"
...
class AgentState(TypedDict):
    ...
    two_hop_search_results: Union[dict, None] # To store details of the second-layer search
    ...
    ablate_indirect_analysis: bool 
    ablate_direct_analysis: bool
...
def run_cisbp_fetch(protein_name: str, cisbp_tf_info_df: pd.DataFrame, cisbp_pwm_dir: str) -> Dict:
    ...
    try:
        ...
        selected_matches_df = protein_matches
        if results['num_total_found'] > MAX_MOTIFS_PER_TF_TO_PROCESS:
            print(f"  Applying advanced filtering to select up to {MAX_MOTIFS_PER_TF_TO_PROCESS} motifs for '{query_protein_name}'.")
            results['filtering_applied'] = True

            # --- Start of new, detailed motif selection logic ---

            # 1. Prepare a working dataframe with columns for sorting
            work_df = protein_matches.copy()
            work_df['is_chip'] = work_df['Motif_Type'].str.lower() == 'chip-seq'
            work_df['year_num'] = pd.to_numeric(work_df['MSource_Year'], errors='coerce').fillna(0)
            status_map = {'D': 0, 'I': 1}
            work_df['status_priority'] = work_df['TF_Status'].map(status_map).fillna(2)
            work_df['is_hocomoco'] = work_df['MSource_Identifier'].str.lower() == 'hocomoco'
            work_df['is_jaspar'] = work_df['MSource_Identifier'].str.lower() == 'jaspar'
            
            selected_motif_ids = []

            # 2. Strategy 1: Select "latest_chip_seq"
            # This prioritizes ChIP-seq motifs, then the most recent, then Direct status.
            latest_chip_seq_df = work_df.sort_values(
                by=['is_chip', 'year_num', 'status_priority'],
                ascending=[False, False, True]
            )
            if not latest_chip_seq_df.empty:
                motif_to_add = latest_chip_seq_df.iloc[0]
                selected_motif_ids.append(motif_to_add['Motif_ID'])
                print(f"    - Selected by latest_chip_seq: {motif_to_add['Motif_ID']} (Year: {motif_to_add['year_num']:.0f}, Chip: {motif_to_add['is_chip']})")

            # 3. Strategy 2: Select one from HOCOMOCO (if available and not already chosen)
            hocomoco_df = work_df[work_df['is_hocomoco'] & ~work_df['Motif_ID'].isin(selected_motif_ids)]
            if not hocomoco_df.empty:
                best_hocomoco = hocomoco_df.sort_values(by='status_priority', ascending=True).iloc[0]
                selected_motif_ids.append(best_hocomoco['Motif_ID'])
                print(f"    - Selected by HOCOMOCO: {best_hocomoco['Motif_ID']} (Status: {best_hocomoco['TF_Status']})")

            # 4. Strategy 3: Select one from JASPAR (if available and not already chosen)
            if len(selected_motif_ids) < MAX_MOTIFS_PER_TF_TO_PROCESS:
                jaspar_df = work_df[work_df['is_jaspar'] & ~work_df['Motif_ID'].isin(selected_motif_ids)]
                if not jaspar_df.empty:
                    best_jaspar = jaspar_df.sort_values(by='status_priority', ascending=True).iloc[0]
                    selected_motif_ids.append(best_jaspar['Motif_ID'])
                    print(f"    - Selected by JASPAR: {best_jaspar['Motif_ID']} (Status: {best_jaspar['TF_Status']})")

            # 5. Fallback: Fill remaining slots with the best available motifs (Direct > Inferred)
            if len(selected_motif_ids) < MAX_MOTIFS_PER_TF_TO_PROCESS:
                num_needed = MAX_MOTIFS_PER_TF_TO_PROCESS - len(selected_motif_ids)
                # Exclude already selected motifs from the fallback pool
                fallback_df = work_df[~work_df['Motif_ID'].isin(selected_motif_ids)].copy() # <-- ADD .copy() HERE
                
                # Use the existing priority function for general fallback sorting
                fallback_df['priority'] = fallback_df.apply(_calculate_motif_priority, axis=1)
                best_fallback = fallback_df.nsmallest(num_needed, 'priority', keep='first')
                
                for _, fallback_motif in best_fallback.iterrows():
                    selected_motif_ids.append(fallback_motif['Motif_ID'])
                    print(f"    - Selected by Fallback: {fallback_motif['Motif_ID']} (Priority: {fallback_motif['priority']})")

            # Create the final dataframe based on the uniquely selected IDs
            final_selected_ids = list(dict.fromkeys(selected_motif_ids))
            selected_matches_df = protein_matches[protein_matches['Motif_ID'].isin(final_selected_ids)].copy()
            # Add a 'priority' column for metadata consistency
            selected_matches_df['priority'] = selected_matches_df.apply(_calculate_motif_priority, axis=1)
            
            print(f"  Selected {len(selected_matches_df)} motifs for '{query_protein_name}' after applying advanced strategy.")        
...
def run_motif_scan(dna_sequence: str, motifs_with_metadata: List[tuple], pvalue_threshold: float) -> Dict:
    ...
    try:
        with tempfile.NamedTemporaryFile(mode='w', suffix='.fasta', delete=False) as dna_file, \
            ...
            fimo_command = [fimo_path, '--oc', fimo_output_dir, '--verbosity', '1', '--thresh', str(pvalue_threshold), motif_filepath, dna_filepath]; print(f"  Running FIMO command: {' '.join(fimo_command)}")
...
def run_string_query(protein_name: str, species_id: int, min_score: int, max_interactors: int) -> Dict:
    ...
    interactors = []
    request_url = "/".join([STRING_API_URL, STRING_OUTPUT_FORMAT, "interaction_partners"])
    status = 'error_string_request_failed'
    queried_name = protein_name

    # Try alias first (if available), then fall back to original name
    alias_name = PROTEIN_ALIASES.get(protein_name, None)
    query_candidates = []
    if alias_name and alias_name != protein_name:
        query_candidates.append(alias_name)
    query_candidates.append(protein_name)

    last_exception = None
    for qname in query_candidates:
        try:
            print(f"  Attempting STRING query for '{qname}'")
            response = requests.get(request_url, params={"identifiers": qname, "species": species_id, "required_score": min_score, "limit": max_interactors * 2}, timeout=45)
            ...
            if data:
                ...
                temp_interactors = []
                for item in data:
                    interactor_name = item.get('preferredName_B')
                    score = item.get('score')
                    string_id_b = item.get('stringId_B')
                    if interactor_name and score is not None and string_id_b:
                        temp_interactors.append({"name": interactor_name, "score": score, "string_id": string_id_b})
                        processed_count += 1
                    else:
                        print(f"  Warn: Skipping STRING entry due to missing data: {item}", file=sys.stderr)
                temp_interactors.sort(key=lambda x: x['score'], reverse=True)
                temp_interactors = temp_interactors[:max_interactors]
                if temp_interactors:
                    interactors = temp_interactors
                    print(f"  Found {len(interactors)} interactors (processed {processed_count}) passing threshold for '{qname}'.")
                    break  # success, stop trying fallbacks
                else:
                    print(f"  No interactors above threshold found for '{qname}'.")
            else:
                print(f"  No interactors returned by STRING DB for '{qname}' above score {min_score}.")
            # If we reach here, this query returned no usable interactors -> try next candidate
        except requests.exceptions.Timeout:
            print(f"  ERROR: STRING DB request timed out for '{qname}'.", file=sys.stderr)
            status = 'error_string_timeout'
            last_exception = sys.exc_info()
            # On timeout, try next candidate (alias/original) before giving up
        except requests.exceptions.RequestException as e:
            status_code = e.response.status_code if hasattr(e, 'response') and e.response is not None else 'N/A'
            print(f"  ERROR querying STRING DB for '{qname}' (Status: {status_code}): {e}", file=sys.stderr)
            status = f'error_string_http_{status_code}'
            last_exception = e
            # For HTTP errors, attempt fallback name if available, otherwise break
        except json.JSONDecodeError as e:
            print(f"  ERROR decoding STRING DB JSON response for '{qname}': {e}", file=sys.stderr)
            status = 'error_string_json_decode'
            last_exception = e
        except Exception as e:
            print(f"  Unexpected ERROR during STRING DB query for '{qname}': {e}", file=sys.stderr)
            traceback.print_exc()
            status = 'error_string_unknown'
            last_exception = e
        # continue to next candidate if any

    # If alias was used and produced no interactors but original might have, the loop already handled it.
    return {'status': status, 'interactors': interactors, 'queried_name': queried_name}
...
def get_protein_input_strategy_for_unknown(
    ...
    if model_type not in _dataset_inference_cache:
        if verbose_matching: print(f"INFO: Initializing ProteinDNADatasetInference for model type '{model_type}' for unknown handling...")
        ...
        if verbose_matching: print(f"INFO: ProteinDNADatasetInference for '{model_type}' initialized.")
...
def run_deepsea_prediction(
    ...
    dna_seq_len: int, device: torch.device, protein_strategy_cache: dict
) -> Dict:
    ...
    try:
        ...
        if model_internal_id is not None:
            ...
            # 1. Check the pre-computed cache first
            if protein_name_uc in protein_strategy_cache:
                protein_input_payload, strategy_type = protein_strategy_cache[protein_name_uc]
                strategy_message = f"Unknown '{protein_name_uc}' for deepsea: Using pre-computed Strategy='{strategy_type}'."
            ...
            if strategy_type == "id":
                ...
                final_logit = deepsea_model(dna_tensor_batch, protein_id_tensor, protein_input_type="id").squeeze()
            elif strategy_type in ["embedding", "tf_family_embedding"]:
...
def run_sei_prediction(
    ...
    try:
        ...
        if model_internal_id is not None:
            ...
            if protein_name_uc in protein_strategy_cache:
                protein_input_payload, strategy_type = protein_strategy_cache[protein_name_uc]
                strategy_message = f"Unknown '{protein_name_uc}' for sei: Using pre-computed Strategy='{strategy_type}'."
...
def get_deepsea_pred_node_wrapper(
    ...
    deepsea_model_specific_resources_uh,
    global_sequences_map_uh,
    tf_family_map_uh,
    deepsea_dna_seq_len_param,
    deepsea_device_param,
    deepsea_protein_strategy_cache
) -> AgentState:
    ...
    try:
        results = run_deepsea_prediction(
            ...
            protein_strategy_cache=deepsea_protein_strategy_cache
        )
...
def get_sei_pred_node_wrapper(
    ...
    try:
        results = run_sei_prediction(
            ...
            protein_strategy_cache=sei_protein_strategy_cache
        )
...
def calculate_confidence_score(state: AgentState, prompt_style: str, use_transformer_prob_as_baseline: bool) -> float:
    ...
    # NEW: Get the fetch results to check the status
    direct_fetch_results = state.get("direct_motif_fetch_results", {})
    direct_motif_status = direct_fetch_results.get('status', 'unknown')
    ...
    # NEW LOGIC to differentiate "no known motifs" from "no hits found"
    if direct_motif_status == 'not_found_cisbp' or not direct_motif_status.startswith('success'):
        # This is the "No Known Motifs" or "Fetch Error" case.
        # This is a neutral or very weak signal. We can apply a small penalty if LLM vote=1,
        # and a small boost if LLM vote=0, but much less than a true "missing hit".
        if llm_vote == 1:
            # A very small penalty because there's no positive motif evidence of any kind.
            adj = direct_missing_penalty * 0.25 # Apply only 25% of the penalty
            adjustments.append(f"Direct Motif: No Known Motifs (Vote=1): {adj:.2f}")
            confidence += adj
        elif llm_vote == 0:
            # A very small boost because the absence of any known motifs is consistent with a negative prediction.
            adj = direct_absence_boost * 0.25 # Apply only 25% of the boost
            adjustments.append(f"Direct Motif: No Known Motifs (Vote=0): +{adj:.2f}")
            confidence += adj
    
    # This 'else' block now handles the case where motifs WERE known and successfully fetched.
    else: 
        if llm_vote == 1:
            if direct_found:
                ...
                # ... (code for num_hits_bonus and p_value_bonus remains the same)
                ...
                if low_pvalue_bonus > 0 and direct_best_pvalue is not None and direct_best_pvalue < low_pvalue_threshold:
                    ...
                    confidence += pval_b
            else: # Vote=1, but NO HITS FOUND after a successful scan for KNOWN motifs. This is the stronger negative signal.
                adj = direct_missing_penalty * direct_mod 
                adjustments.append(f"Direct Hit Missing (Vote=1, scanned for known motifs): {adj:.2f}")
                confidence += adj
        elif llm_vote == 0:
            if direct_found:
                ...
                if is_reliable_direct:
                    ...
                    confidence += adj_extra
            else: # Vote=0, and NO HITS FOUND after a successful scan for KNOWN motifs. This is consistent.
                adj = direct_absence_boost * direct_mod 
                adjustments.append(f"Direct Hit Absence (Vote=0, scanned for known motifs): +{adj:.2f}")
                confidence += adj
...
def summarize_direct_analysis_node(state: AgentState) -> AgentState:
    ...
    if state.get("error"): return state;
    fetch_results = state.get("direct_motif_fetch_results") or {}; scan_results = state.get("direct_motif_scan_results") or {"hits": [], "status": "unknown"}; motifs_metadata_list = [meta for _, meta in fetch_results.get('motifs_metadata', [])]; reliability_str = _calculate_cisbp_motif_reliability_for_prompt(motifs_metadata_list); all_significant_hits = scan_results.get("hits", []); motif_found_in_dna = len(all_significant_hits) > 0
    best_hit = all_significant_hits[0] if motif_found_in_dna else None
    best_score = best_hit.get('score') if best_hit else None
    best_pvalue = best_hit.get('pvalue') if best_hit else None 
    if best_score is not None and math.isnan(best_score):
        best_score = None
    if best_pvalue is not None and math.isnan(best_pvalue):
        best_pvalue = None    
    scan_status = scan_results.get('status', 'unknown')
    scan_message = scan_results.get('message')
...
def query_string_node(state: AgentState) -> AgentState:
    ...
    if state.get("error"): return state
    try: results = run_string_query(protein_name=state["protein_name"], species_id=TARGET_SPECIES_TAXONOMY_ID, min_score=args.string_min_score, max_interactors=args.max_string_interactors); return {**state, "string_interactors": results.get('interactors', [])}
...
def filter_string_interactors_node(state: AgentState, cisbp_tf_info_df: pd.DataFrame, cisbp_pwm_dir: str, enable_two_hop: bool) -> AgentState:
    print("\n--- Node: Filter and Validate STRING Interactors for Analyzable TFs ---")
    if state.get("error"): return state
    
    original_protein_name = state["protein_name"]
    string_interactors = state.get("string_interactors", [])
    tfs_for_analysis = []
    all_tf_interactors_layer1 = []
    two_hop_results = {"status": "not_triggered", "bridge_protein": None, "found_tfs": []}
    max_to_analyze = args.max_interacting_tfs

    if not string_interactors or cisbp_tf_info_df is None or cisbp_tf_info_df.empty:
        print("  Skipping: No interactors or CisBP data unavailable.")
        return {**state, "tf_interactors_cisbp": [], "tfs_analyzed_indirectly": [], "two_hop_search_results": two_hop_results}

    try:
        # --- Layer 1 Search (Existing Logic) ---
        with open(args.string_tf_list_path, 'r') as f:
            provided_tf_names = {line.strip() for line in f if line.strip()}
        
        all_tf_interactors_layer1 = [i for i in string_interactors if i.get("name") in provided_tf_names]
        all_tf_interactors_layer1.sort(key=lambda x: x.get('score', 0), reverse=True)
        print(f"  Layer 1: Found {len(all_tf_interactors_layer1)} known TFs. Validating up to {max_to_analyze}...")

        for tf_candidate in all_tf_interactors_layer1:
            if len(tfs_for_analysis) >= max_to_analyze: break
            # (Validation logic remains the same)
            motif_entries = cisbp_tf_info_df[cisbp_tf_info_df['TF_Name'] == tf_candidate.get("name")]
            if any(os.path.exists(os.path.join(cisbp_pwm_dir, f"{mid}.txt")) and os.path.getsize(os.path.join(cisbp_pwm_dir, f"{mid}.txt")) > 10 for mid in motif_entries['Motif_ID']):
                tfs_for_analysis.append(tf_candidate)
                print(f"    + Layer 1: Found valid motif for '{tf_candidate.get('name')}'. Added to list.")

        # --- Layer 2 Search (New Fallback Logic) ---
        if len(tfs_for_analysis) < max_to_analyze and enable_two_hop:
            print(f"\n  Layer 1 search found only {len(tfs_for_analysis)}/{max_to_analyze} analyzable TFs. Activating two-hop search.")
            two_hop_results["status"] = "triggered_no_results"
            
            # This set prevents analyzing the original protein or any of the Layer 1 TFs again
            excluded_proteins = {original_protein_name} | {tf['name'] for tf in all_tf_interactors_layer1}

            for bridge_tf in all_tf_interactors_layer1: # e.g., RAD21, SMC3...
                if len(tfs_for_analysis) >= max_to_analyze: break
                print(f"    Searching for interactors of bridge protein: '{bridge_tf['name']}'")
                
                # Run a new STRING query for the bridge TF
                layer2_query_result = run_string_query(bridge_tf['name'], TARGET_SPECIES_TAXONOMY_ID, args.string_min_score, args.max_string_interactors)
                layer2_interactors = layer2_query_result.get('interactors', [])
                
                if not layer2_interactors: continue

                for l2_candidate in layer2_interactors:
                    l2_name = l2_candidate.get("name")
                    if not l2_name or l2_name in excluded_proteins or l2_name not in provided_tf_names:
                        continue # Skip if it's not a TF, is excluded, or has no name

                    # Validate this new Layer 2 TF
                    motif_entries = cisbp_tf_info_df[cisbp_tf_info_df['TF_Name'] == l2_name]
                    if any(os.path.exists(os.path.join(cisbp_pwm_dir, f"{mid}.txt")) and os.path.getsize(os.path.join(cisbp_pwm_dir, f"{mid}.txt")) > 10 for mid in motif_entries['Motif_ID']):
                        print(f"      + Layer 2: Found valid motif for '{l2_name}'. Adding to list.")
                        tfs_for_analysis.append(l2_candidate)
                        
                        # Log the successful find
                        two_hop_results["status"] = "success"
                        two_hop_results["bridge_protein"] = bridge_tf['name']
                        two_hop_results["found_tfs"].append(l2_candidate)
                        
                        excluded_proteins.add(l2_name) # Add to exclusion list to avoid duplicates
                        if len(tfs_for_analysis) >= max_to_analyze: break
        
    except Exception as e:
        print(f"  ERROR during TF interactor filtering: {e}", file=sys.stderr)
        traceback.print_exc()

    print(f"\n  Final list for analysis contains {len(tfs_for_analysis)} validated TFs.")
    return {**state, "tf_interactors_cisbp": all_tf_interactors_layer1, "tfs_analyzed_indirectly": tfs_for_analysis, "two_hop_search_results": two_hop_results}
...
def summarize_indirect_analysis_node(state: AgentState) -> AgentState:
    ...
    if state.get("error"): return state;
    string_interactors = state.get("string_interactors") or []; tf_interactors_cisbp = state.get("tf_interactors_cisbp") or []; tfs_analyzed_indirectly = state.get("tfs_analyzed_indirectly") or []; scan_results = state.get("indirect_motif_scan_results") or {"hits": [], "status": "unknown"}; 

    two_hop_details = state.get("two_hop_search_results")
    
    scan_status = scan_results.get('status', 'unknown'); scan_message = scan_results.get('message'); all_significant_hits = scan_results.get("hits", []); any_indirect_motif_found = len(all_significant_hits) > 0; all_significant_hits.sort(key=lambda x: x.get('pvalue', float('inf')))
    best_hit = all_significant_hits[0] if any_indirect_motif_found else None
    if best_hit:
        best_score = best_hit.get('score')
        best_pvalue = best_hit.get('pvalue')
        if best_score is not None and math.isnan(best_score):
            best_hit['score'] = None
        if best_pvalue is not None and math.isnan(best_pvalue):
            best_hit['pvalue'] = None

    summary = {"search_performed": True, "skipped_reason": None, "interactors_found_string": string_interactors, "interactors_tf_cisbp": tf_interactors_cisbp, "interactors_tf_analyzed": tfs_analyzed_indirectly, "interacting_tf_hits": all_significant_hits, "best_interacting_tf_hit": best_hit, "any_indirect_motif_found": any_indirect_motif_found, "scan_status": scan_status, "scan_message": scan_message, "error_message": None, "two_hop_details": two_hop_details}
    ...
    if state.get("indirect_motif_analysis_summary") and not state["indirect_motif_analysis_summary"].get("search_performed"):
        ...
        print(f"  Indirect Analysis Summary: Search Performed, Hits Found={summary['any_indirect_motif_found']}, #Interactors={len(summary.get('interactors_found_string',[]))}, #TF_Interactors={len(summary.get('interactors_tf_cisbp',[]))}, #TFs_Analyzed={len(summary.get('interactors_tf_analyzed',[]))}, Scan Status={scan_status}");
        if two_hop_details and two_hop_details.get("status") == "success":
            print(f"    -> NOTE: Analysis includes TFs found via two-hop search from bridge protein '{two_hop_details.get('bridge_protein')}'.")
...
def generate_concise_llm_prompt(state: AgentState) -> str:
    ...
    # --- Format Direct Motif Summary (NEW LOGIC HERE) ---
    ...
    direct_motif_summary = ""
    if direct_motif_status == 'not_found_cisbp' or not direct_motif_status.startswith('success'):
        direct_motif_summary = "No Known Motifs in Database"
    elif not direct_scan_status.startswith('success'):
        direct_motif_summary = f"Scan Error ({direct_scan_status})"
    elif direct_motif_found_dna:
        direct_motif_summary = f"Hits Found: YES ({direct_hits_count} hits, Reliability: {direct_reliability_str})"
    else: # Success on fetch and scan, but no hits
        direct_motif_summary = f"Hits Found: NO (Scanned for known motifs, Reliability: {direct_reliability_str})"
...
def _format_common_evidence_for_prompt(state: AgentState) -> Dict[str, str]:
    ...
    # NEW LOGIC: Differentiate between "no known motifs" and "no hits found"
    direct_motif_found_dna = direct_summary.get('motif_found_in_dna', False)
    direct_hits_count = direct_summary.get('number_of_hits', 0)
    direct_reliability_str = direct_summary.get('motif_reliability_source_category', 'Unknown')
    direct_scan_status = direct_summary.get('scan_status', 'unknown')
    
    direct_motif_summary_for_header = ""
    if direct_motif_status == 'not_found_cisbp' or not direct_motif_status.startswith('success'):
        # The outcome is the same: no usable motifs. Present this fact cleanly to the LLM.
        direct_motif_summary_for_header = "No Known Motifs in Database"
        evidence_parts['direct_protein_has_motifs_str'] = "No" 
    elif not direct_scan_status.startswith('success'):
        direct_motif_summary_for_header = f"Scan Error ({direct_scan_status})"
        evidence_parts['direct_protein_has_motifs_str'] = "Yes"
    elif direct_motif_found_dna:
        direct_motif_summary_for_header = f"Hits Found: YES ({direct_hits_count} hits, Reliability: {direct_reliability_str})"
        evidence_parts['direct_protein_has_motifs_str'] = "Yes"
    else: # This now implies success, but no hits
        direct_motif_summary_for_header = f"Hits Found: NO (Scanned for known motifs, Reliability: {direct_reliability_str})"
        evidence_parts['direct_protein_has_motifs_str'] = "Yes" # The protein has motifs, they just weren't found here
   
    # This new summary string will be used in the concise prompt.
    evidence_parts['direct_motif_summary_for_header'] = direct_motif_summary_for_header

    # For verbose prompt fields (this logic is mostly okay but let's refine it)
    evidence_parts['direct_reliability_str'] = direct_reliability_str # keep this for consistency
    evidence_parts['direct_motif_found_dna_str'] = str(direct_motif_found_dna)
    ...
    if direct_motif_status == 'not_found_cisbp' or not direct_motif_status.startswith('success'):
        direct_cisbp_search_details_list.append(" The protein has no known direct binding motifs in the CisBP database.")
    elif direct_motif_status.startswith('success'):
    ...
    direct_scan_status = direct_summary.get('scan_status', 'unknown')
    if direct_motif_status == 'not_found_cisbp' or not direct_motif_status.startswith('success'):
         direct_scan_info_list.append(f"  Scan not performed because no known motifs were available for the protein.")
    elif direct_scan_status.startswith('error'):
        ...
        direct_scan_info_list.append(f"  No significant direct motif hits found (p <= {p_thresh_print}) after scanning with known motifs. Scan status: {direct_scan_status}")
    ...
    if not indirect_search_performed and evidence_parts['indirect_skipped_reason_str']: indirect_info_list.append(f"  Indirect search skipped: {evidence_parts['indirect_skipped_reason_str']}")
        ...
        # NEW: Check for and report two-hop search details
        two_hop_info = indirect_summary.get("two_hop_details")
        if two_hop_info and two_hop_info.get("status") == "success":
            bridge_protein = two_hop_info.get("bridge_protein", "an interactor")
            indirect_info_list.append(f"    - NOTE: Some TFs were found via a 'two-hop' search through the bridge protein '{bridge_protein}'.")
        ...
        indirect_info_list.append(f"  STRING DB Search: Found {string_interactors_count} interactors (Score > {args.string_min_score}).")
...
1.  **Direct Evidence:** Evaluate the direct motif evidence. Does the protein have known binding motifs? **Crucially, distinguish between a protein having "No Known Motifs" (a lack of information) versus having known motifs that were scanned for but "Not Found" in the DNA (a more negative signal).** Are they reliable? Were any significant hits found in the DNA sequence? Assess the strength of this evidence (strong positive, moderate positive, neutral, weak negative, strong negative, or error).
2.  **Indirect Evidence:** Evaluate the indirect motif evidence. Did the protein interact with known TFs? Were any motifs belonging to these interacting TFs found in the DNA sequence? Assess the strength of this evidence, considering it's less direct than the protein's own motifs.
...
def should_call_llm_or_predict_directly(state: AgentState) -> str:
    """
    Checks if fast screening mode is enabled to decide the next step.
    This function now correctly accesses the 'args' object passed at runtime.
    """
    # This is a placeholder check. The actual check needs access to the args object.
    # We will handle this by checking a flag passed into the graph execution context
    # or by making args globally accessible (less ideal but simple for a script).
    # For this demonstration, we assume 'args.fast_screen' is accessible.
    print("\n--- Edge Logic: should_call_llm_or_predict_directly ---")
    if state.get("error"):
        return "error_path"

    # NOTE: This requires 'args' to be in the scope.
    # It is in a __main__ block, so we pass it into the graph compilation step.
    if args.fast_screen:
        print("  --> Fast screen enabled. Routing to direct_prediction.")
        return "predict_directly"
    else:
        print("  --> Fast screen disabled. Routing to generate_llm_prompt.")
        return "call_llm"

def direct_prediction_node(state: AgentState) -> AgentState:
    """
    Bypasses the LLM and makes a direct prediction. This version implements a clearer
    evidence fallback hierarchy: Direct Motifs > Indirect Motifs > Model Predictions.
    It handles ablated evidence and nuanced multi-model interpretation.
    """
    print("\n--- Node: Direct Prediction (Fast Screen Mode) ---")
    if state.get("error"): return state

    style = state.get("llm_prompt_style_selected", "verbose")
    print(f"  Using rule-based logic derived from '{style}' prompt style.")

    # --- 1. Gather and Interpret All Evidence (No changes in this section) ---
    direct_summary = state.get("direct_motif_analysis_summary", {})
    direct_analysis_ablated = "ablation" in direct_summary.get('scan_status', '')
    direct_found = direct_summary.get('motif_found_in_dna', False) and not direct_analysis_ablated
    direct_reliability = direct_summary.get('motif_reliability_source_category', 'Unknown')
    direct_scan_ok = not direct_summary.get('scan_status', 'unknown').startswith('error') and not direct_analysis_ablated
    direct_motif_known_and_scanned_but_not_found = (
        not direct_summary.get('protein_motif_info', {}).get('status') == 'not_found_cisbp'
        and direct_scan_ok
        and not direct_found
    )

    indirect_summary = state.get("indirect_motif_analysis_summary", {})
    indirect_analysis_ablated = "ablation" in indirect_summary.get('scan_status', '')
    indirect_found = indirect_summary.get('any_indirect_motif_found', False) and not indirect_analysis_ablated

    all_predictions = state.get("all_model_predictions", [])
    successful_models = [p for p in all_predictions if p.get("probability") is not None and not p.get("status", "").startswith("error")]
    num_models = len(successful_models)
    
    model_consensus_summary = {
        "num_models": num_models, "avg_prob": 0.5, "avg_certainty": 0.0,
        "pos_votes": 0, "neg_votes": 0, "is_conflict": False, "final_vote": 0
    }

    if num_models > 0:
        probs = [p['probability'] for p in successful_models]
        model_consensus_summary["avg_prob"] = sum(probs) / num_models
        model_consensus_summary["avg_certainty"] = sum(abs(p - 0.5) for p in probs) / num_models
        model_consensus_summary["pos_votes"] = sum(1 for p in probs if p > 0.5)
        model_consensus_summary["neg_votes"] = num_models - model_consensus_summary["pos_votes"]
        model_consensus_summary["is_conflict"] = model_consensus_summary["pos_votes"] > 0 and model_consensus_summary["neg_votes"] > 0
        model_consensus_summary["final_vote"] = 1 if model_consensus_summary["avg_prob"] > 0.5 else 0

    # --- 2. Apply Rules Based on Style ---
    final_vote = 0
    confidence = 0.5
    explanation_steps = []

    if style == "motif-priority":
        explanation_steps.append("1. Strategy: Motif-Priority.")
        if direct_found and "High" in direct_reliability:
            final_vote = 1; confidence = 0.90
            explanation_steps.append("2. Decisive Evidence: Strong, reliable direct motif hit found.")
        elif direct_motif_known_and_scanned_but_not_found:
            final_vote = 0; confidence = 0.90
            explanation_steps.append("2. Decisive Evidence: Known direct motifs were scanned but NOT found (strong negative signal).")
        
        # +++ NEW FALLBACK LOGIC HIERARCHY +++
        elif indirect_found and not indirect_analysis_ablated:
            final_vote = 1; confidence = 0.70 # Confidence is positive but less than a direct hit.
            explanation_steps.append("2. Direct evidence inconclusive. Falling back to indirect evidence.")
            explanation_steps.append("3. Decisive Evidence: A positive indirect motif hit was found.")
        # +++ END OF NEW LOGIC +++

        elif num_models > 0:
            explanation_steps.append("2. Direct and Indirect motif evidence is inconclusive; falling back to model consensus.")
            if not model_consensus_summary["is_conflict"] and model_consensus_summary["avg_certainty"] > 0.3:
                final_vote = model_consensus_summary["final_vote"]
                confidence = 0.6 + model_consensus_summary["avg_certainty"]
                explanation_steps.append(f"3. Models show confident, unanimous agreement (Avg Certainty: {model_consensus_summary['avg_certainty']:.2f}).")
            else:
                final_vote = model_consensus_summary["final_vote"]
                confidence = 0.5 + model_consensus_summary["avg_certainty"] / 2
                explanation_steps.append(f"3. Models are uncertain or in conflict. Relying on weak model lean (Avg Prob: {model_consensus_summary['avg_prob']:.3f}).")
        else:
            final_vote = 0; confidence = 0.40
            explanation_steps.append("2. No conclusive motif or model evidence. Defaulting to 0.")

    elif style == "transformer-priority": # i.e., Model-Priority
        explanation_steps.append("1. Strategy: Model-Priority.")
        if num_models > 0 and not model_consensus_summary["is_conflict"] and model_consensus_summary["avg_certainty"] > 0.3:
            final_vote = model_consensus_summary["final_vote"]; confidence = 0.90
            explanation_steps.append(f"2. Decisive Evidence: Models show strong, unanimous agreement (Avg Certainty: {model_consensus_summary['avg_certainty']:.2f}).")
        
        # +++ MODIFIED FALLBACK LOGIC FOR MODEL CONFLICT/WEAKNESS +++
        elif num_models > 0 and model_consensus_summary["is_conflict"]:
             explanation_steps.append("2. Models are in conflict. Overriding models and falling back to motif evidence hierarchy (Direct > Indirect).")
             if direct_found:
                 final_vote = 1; confidence = 0.75
                 explanation_steps.append("3. Found direct motif hit, providing deciding evidence.")
             elif indirect_found and not indirect_analysis_ablated:
                 final_vote = 1; confidence = 0.65
                 explanation_steps.append("3. No direct hit, but found indirect motif hit, providing deciding evidence.")
             else:
                 final_vote = 0; confidence = 0.60
                 explanation_steps.append("3. No positive motif evidence found. Defaulting to 0.")
        elif (direct_found and "High" in direct_reliability) or (indirect_found and not indirect_analysis_ablated): # Models are weak/unanimous, but motifs are present
            explanation_steps.append("2. Models were weak/unanimous. Falling back to motif evidence hierarchy.")
            if direct_found and "High" in direct_reliability:
                final_vote = 1; confidence = 0.85
                explanation_steps.append("3. Decisive evidence from strong, reliable direct motif hit.")
            else: # Must be indirect_found
                final_vote = 1; confidence = 0.70
                explanation_steps.append("3. Decisive evidence from an indirect motif hit.")
        # +++ END OF MODIFICATION +++

        elif num_models > 0: # Models are unanimous but not very confident, and no motif hits
            final_vote = model_consensus_summary["final_vote"]
            confidence = 0.5 + model_consensus_summary["avg_certainty"]
            explanation_steps.append(f"2. No decisive motif evidence. Relying on the weak lean of the model consensus (Avg Prob: {model_consensus_summary['avg_prob']:.3f}).")
        else: # No models, rely on motifs
            explanation_steps.append("2. No model data available; relying on motif evidence hierarchy.")
            if direct_found: final_vote = 1; confidence = 0.75
            elif indirect_found: final_vote = 1; confidence = 0.65
            else: final_vote = 0; confidence = 0.60
            explanation_steps.append(f"3. Motif evidence points to vote {final_vote}.")

    else: # Balanced approach (no changes here)
        explanation_steps.append("1. Strategy: Balanced.")
        score = 0
        explanation_steps.append("2. Scoring evidence:")
        if num_models > 0:
            model_score = (model_consensus_summary["avg_prob"] - 0.5) * 2.0
            if not model_consensus_summary["is_conflict"]: model_score *= 1.5
            else: model_score *= 0.5
            score += model_score
            explanation_steps.append(f" - Models (score: {model_score:.2f}, conflict: {model_consensus_summary['is_conflict']})")
        if not direct_analysis_ablated:
            if direct_found: score += 1.5; explanation_steps.append(" - Direct Hit Found (+1.5)")
            if direct_motif_known_and_scanned_but_not_found: score -= 1.5; explanation_steps.append(" - Direct Hit Missing (-1.5)")
        if not indirect_analysis_ablated:
            if indirect_found: score += 0.5; explanation_steps.append(" - Indirect Hit Found (+0.5)")

        final_vote = 1 if score > 0 else 0
        confidence = 0.5 + min(0.49, abs(score) / 4.0)
        explanation_steps.append(f"3. Final Score: {score:.2f} (Threshold > 0 for vote=1).")

    final_explanation = "[Fast Screen Prediction] " + " ".join(explanation_steps)
    print(f"  {final_explanation}")

    return {
        **state,
        "llm_vote": final_vote,
        "llm_explanation": final_explanation,
        "raw_llm_response": "N/A (Fast Screen Mode)",
        "final_confidence": max(0.0, min(1.0, confidence))
    }

def prune_final_state_for_logging(final_state: dict) -> dict:
    """
    Removes redundant information from the final state dictionary before logging
    to make the comprehensive output more concise.
    """
    print("  Pruning redundant keys from final state for comprehensive log...")
    
    # Make a copy to avoid modifying the original state object in memory
    pruned_state = final_state.copy()

    # 1. Remove top-level keys that are fully summarized in `indirect_motif_analysis_summary`
    keys_to_remove = [
        "string_interactors",
        "tf_interactors_cisbp",
        "tfs_analyzed_indirectly",
        "indirect_motif_scan_results" # This is summarized within the analysis summary
    ]
    for key in keys_to_remove:
        if key in pruned_state:
            del pruned_state[key]

    # 2. Prune the verbose raw PWM data from `indirect_motif_fetch_results`
    if "indirect_motif_fetch_results" in pruned_state and pruned_state["indirect_motif_fetch_results"]:
        # We want to keep the metadata, just not the giant PWM arrays
        if "motifs_metadata" in pruned_state["indirect_motif_fetch_results"]:
            # This field contains tuples of (pwm_dict, metadata_dict)
            # We will replace it with a list of just the metadata_dict
            metadata_only = [meta for pwm, meta in pruned_state["indirect_motif_fetch_results"]["motifs_metadata"]]
            pruned_state["indirect_motif_fetch_results"]["motifs_metadata"] = metadata_only
            pruned_state["indirect_motif_fetch_results"]["status"] += "_pwm_pruned" # Add a note

    # 3. Prune raw PWM data from direct results as well for consistency
    if "direct_motif_fetch_results" in pruned_state and pruned_state["direct_motif_fetch_results"]:
         if "motifs_metadata" in pruned_state["direct_motif_fetch_results"]:
            metadata_only_direct = [meta for pwm, meta in pruned_state["direct_motif_fetch_results"]["motifs_metadata"]]
            pruned_state["direct_motif_fetch_results"]["motifs_metadata"] = metadata_only_direct

    # 4. The direct_motif_analysis_summary also contains a copy of direct_motif_fetch_results. Prune it too.
    if "direct_motif_analysis_summary" in pruned_state and "protein_motif_info" in pruned_state["direct_motif_analysis_summary"]:
        if "motifs_metadata" in pruned_state["direct_motif_analysis_summary"]["protein_motif_info"]:
             metadata_only_summary = [meta for pwm, meta in pruned_state["direct_motif_analysis_summary"]["protein_motif_info"]["motifs_metadata"]]
             pruned_state["direct_motif_analysis_summary"]["protein_motif_info"]["motifs_metadata"] = metadata_only_summary

    return pruned_state

def batch_determine_protein_strategies(
    protein_names_uc: list[str],
    model_type: str,
    model_specific_resources: dict,
    global_protein_sequences_map_arg: dict,
    protein_id_to_tf_family_map_shared_arg: dict,
    disabled_fallback_strategies_cfg: list = None # New argument
) -> dict:
    """
    Takes a list of novel protein names and batch-determines their handling strategy
    by calling the ProteinDNADatasetInference handler for each.
    Returns a dictionary mapping protein names to their (payload, strategy) tuple.
    """
    # This reuses the same setup logic from get_protein_input_strategy_for_unknown
    ...
    if model_type not in _dataset_inference_cache:
        # Create a temporary instance for batch processing
        _dataset_inference_cache[model_type] = ProteinDNADatasetInference(
            data_df=pd.DataFrame(), protein_id_to_model_id_map=model_specific_resources["protein_id_to_model_id_map"],
            dna_seq_len=0, global_protein_sequences_map=global_protein_sequences_map_arg,
            protein_id_to_embedding_map=model_specific_resources["protein_id_to_embedding_map_for_fallback"],
            ...
            tf_family_to_known_protein_ids_map=model_specific_resources["tf_family_to_model_known_protein_ids_map"],
            fallback_embedding_for_unknown=model_specific_resources["fallback_cpu_embedding"],
            verbose_unknown_matching=args.verbose_debug_unknown_handling,
            num_top_similar_for_avg_logits=NUM_TOP_SIMILAR_FOR_AVG_LOGITS_CONFIG,
            similarity_engine=SIMILARITY_ENGINE_CONFIG,
            max_levenshtein_dist=MAX_LEVENSHTEIN_DIST_CONFIG,
            min_mmseqs_blast_bitscore=MIN_MMSEQS_BLAST_BITSCORE_CONFIG,
            max_mmseqs_blast_evalue=MAX_MMSEQS_BLAST_EVALUE_CONFIG,
            disabled_fallback_strategies=disabled_fallback_strategies_cfg,
            mmseqs_db_path_prefix=model_specific_resources["mmseqs_db_path_prefix"],
            blast_db_path_prefix=model_specific_resources["blast_db_path_prefix"],
            temp_dir_base=temp_dir_base_path_cfg
        )
    ...
    # Process the list of proteins
    strategy_results = {}
    desc = f"Batch pre-computing strategies for {model_type.upper()} novel proteins"
    for protein_name in tqdm(protein_names_uc, desc=desc):
        try:
            payload, strategy_type = dataset_handler._handle_unknown_protein(protein_name)
            strategy_results[protein_name] = (payload, strategy_type)
        except Exception as e:
            print(f"  ERROR pre-computing strategy for '{protein_name}': {e}", file=sys.stderr)
            strategy_results[protein_name] = (None, "error_pre-computation")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Path to the evaluation dataset file (.csv or .tsv or .pkl). Required columns: 'dna', 'protein', 'label'."
    )
    parser.add_argument(
        "--llm-model", type=str,
    ...
    parser.add_argument(
        "--fast-screen",
        action="store_true",
        help="Enable fast screening mode: skips LLM call and generates a direct prediction based on evidence and the effective prompt style."
    )    
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Exclude records belonging to proteins used in training the DPI model."
    )

    # Indirect motif analysis arguments
    parser.add_argument("--max-interacting-tfs", type=int, default=MAX_INTERACTING_TFS_TO_ANALYZE,
                        help="Maximum number of interacting TFs to analyze during indirect motif analysis. Default: 3")
    parser.add_argument("--max-string-interactors", type=int, default=MAX_STRING_INTERACTORS_TO_CHECK,
                        help="Maximum number of STRING interactors to fetch for indirect motif analysis. Default: 50")
    parser.add_argument("--string-min-score", type=int, default=STRING_MIN_INTERACTION_SCORE,
                        help="Minimum STRING interaction score to consider for indirect motif analysis. Default: 700")  
    parser.add_argument("--string-tf-list-path", type=str, default=STRING_TF_LIST_PATH,
                        help="Path to the list of transcription factors for STRING interactor filtering.")
    parser.add_argument("--enable-two-hop-indirect-search", action="store_true",
                        help="Enable two-hop search for indirect TFs if no analyzable TFs are found in the first layer.")    
    # DeepSEA specific arguments
    parser.add_argument("--deepsea-model-path", type=str, default=DEEPSEA_MODEL_PATH, 
                        help="Path to the DeepSEA model .pkl state_dict file.")
    parser.add_argument("--deepsea-protein-map-path", type=str, default=DEEPSEA_PROTEIN_MAP_PATH, 
                        help="Path to DeepSEA protein_to_id_map.json file.")
    ...
    parser.add_argument("--sei-model-path", type=str, default=SEI_MODEL_PATH, 
                        help="Path to the SEI model .pkl state_dict file.")
    parser.add_argument("--sei-protein-map-path", type=str, default=SEI_PROTEIN_MAP_PATH, 
                        help="Path to SEI protein_to_id_map.json file.")
    ...
    parser.add_argument("--verbose-debug-unknown-handling", action="store_true", help="Enable verbose logging from ProteinDNADatasetInference during unknown handling.")
    # DPI specific arguments ---
    parser.add_argument("--dpi-model-path", type=str, default=TRANSFORMER_MODEL_PATH, 
                        help="Path to the DPI model .pt state_dict file.")
    parser.add_argument("--dpi-train-protein-path", type=str, default=TRAINING_PROTEIN_LIST_PATH, 
                        help="Path to the trained proteins of dpi model.")
    parser.add_argument("--protein-filter-file", type=str, default=None, 
                        help="Path to the protein filter file.")
    parser.add_argument("--disabled-fallback-strategies", type=str, default="", 
                        help="Comma-separated list of fallback strategies to disable for unknown proteins. Choices: 'ensemble_similar_ids', 'tf_family_embedding'. E.g., 'ensemble_similar_ids,tf_family_embedding'.")
    args = parser.parse_args()

    # --- Parse and validate the new argument ---
    disabled_strategies_list = [s.strip().lower() for s in args.disabled_fallback_strategies.split(',') if s.strip()]
    valid_strategies = {"ensemble_similar_ids", "tf_family_embedding"}
    for strategy in disabled_strategies_list:
        if strategy not in valid_strategies:
            parser.error(f"Invalid strategy '{strategy}' in --disabled-fallback-strategies. Valid choices are: {list(valid_strategies)}")
    
    selected_llm_safe = re.sub(r'[^\w\-_\.]', '_', args.llm_model.replace('/', '_'))
    llm_name_prefix = f"{selected_llm_safe}_" if not args.fast_screen else ""
    fast_screen_suffix = "_fast_screen" if args.fast_screen else ""
    llm_name_part_for_filename = f"_{selected_llm_safe}" if not args.fast_screen else ""
    ...
    output_subdir_name_for_run = f"{llm_name_prefix}prompt_{args.prompt_style}_models_{models_run_str_safe}{ablation_subdir_suffix}{fast_screen_suffix}"
    ...
    # --- NEW: Automatically generate unique DB paths based on config hashes ---
    # This prevents stale cache issues.
    print("--- Generating unique database paths based on configuration hashes ---")
    for model_type in models_to_run_arg:
        if model_type in ['deepsea', 'sei']:
            # 1. Get the specific config paths that define the DB content
            protein_map_path = getattr(args, f"{model_type}_protein_map_path", None)
            fasta_path = FASTA_FILE_PATH_CONFIG

            if not protein_map_path:
                continue

            # 2. Create a unique string from these dependencies
            dependency_string = f"{protein_map_path}{fasta_path}"
            
            # 3. Hash the string to create a short, unique fingerprint
            hasher = hashlib.sha256(dependency_string.encode('utf-8'))
            hash_prefix = hasher.hexdigest()[:12] # Use the first 12 chars of the hash

            # 4. Construct the new, unique path prefixes for this specific config
            unique_db_dir = UNKNOWN_HANDLING_TEMP_DIR_BASE_CONFIG / f"{model_type}_{hash_prefix}"
            unique_db_dir.mkdir(parents=True, exist_ok=True)
            
            mmseqs_prefix = unique_db_dir / "known_mmseqs_db"
            blast_prefix = unique_db_dir / "known_blast_db"
            
            print(f"  Hashed path for '{model_type}' DBs: {unique_db_dir}")

            # 5. Prepare the MODEL_SPECIFIC_RESOURCES dictionary which will be used later
            if model_type not in MODEL_SPECIFIC_RESOURCES_FOR_UNKNOWN_HANDLING:
                MODEL_SPECIFIC_RESOURCES_FOR_UNKNOWN_HANDLING[model_type] = {}
            
            MODEL_SPECIFIC_RESOURCES_FOR_UNKNOWN_HANDLING[model_type]["mmseqs_db_path_prefix"] = mmseqs_prefix
            MODEL_SPECIFIC_RESOURCES_FOR_UNKNOWN_HANDLING[model_type]["blast_db_path_prefix"] = blast_prefix
    ...
    if "deepsea" in models_to_run_arg:
        ...
        MODEL_SPECIFIC_RESOURCES_FOR_UNKNOWN_HANDLING["deepsea"].update({
    ...
    if "sei" in models_to_run_arg:
        ...
        MODEL_SPECIFIC_RESOURCES_FOR_UNKNOWN_HANDLING["sei"].update({
    ...
    DNA_EMB_DB_PATH = '/new-stg/home/cong/DPI/dataset/Encode3and4/deepsea/embeddings/test_min10/dna_embeddings.duckdb'
    ...
    if "dpi" in models_to_run_arg: # DPI training protein list
        try:
            if os.path.exists(args.dpi_train_protein_path):
                 with open(args.dpi_train_protein_path, 'r') as f: training_proteins = [line.strip() for line in f if line.strip()]
                 training_protein_set = set(training_proteins); print(f"Loaded {len(training_protein_set)} DPI training proteins.")
            else: print(f"WARN: DPI Training protein list not found: {args.dpi_train_protein_path}", file=sys.stderr)
    ...
    if "dpi" in models_to_run_arg: # DPI model itself
        try:
            ...
            matches = re.findall(r'([a-zA-Z_]+)=([\d.]+)', os.path.basename(args.dpi_model_path)); parsed = {key: (int(v) if v.isdigit() else float(v) if '.' in v else v) for key, v in matches}
            ...
            state_dict = torch.load(args.dpi_model_path, map_location=DEVICE)
    ...
    try:
        ...
        elif args.evaluation_file.endswith('.csv'): df_eval_raw = pd.read_csv(args.evaluation_file); print(f"  Loaded {len(df_eval_raw)} raw samples from csv.")
        else: raise ValueError("Unsupported evaluation file format. Use .pkl or .tsv")
        
        # --- Conditionally filter out training proteins from evaluation set based on argument ---
        if args.exclude_training_proteins:
            print("  --exclude-training-proteins flag is set. Filtering out proteins from the DPI training set.")
            if not training_protein_set:
                print("  WARN: --exclude-training-proteins was set, but the DPI training protein set is empty. No filtering applied.")
            else:
                original_count = len(df_eval_raw)
                # Ensure protein column is string type before using .str accessor to avoid errors on mixed-type columns
                df_eval_raw['protein'] = df_eval_raw['protein'].astype(str)
                df_eval_raw = df_eval_raw[~df_eval_raw['protein'].str.upper().isin(training_protein_set)]
                filtered_count = len(df_eval_raw)
                print(f"  Filtered out {original_count - filtered_count} records belonging to training proteins. {filtered_count} records remain.")
        else:
            print("  --exclude-training-proteins flag not set. Evaluation will include all proteins from the file.")

        # --- NEW: Additional filtering based on user-provided protein list ---
        if args.protein_filter_file:
            print(f"  Applying additional filter from file: {args.protein_filter_file}")
            try:
                with open(args.protein_filter_file, 'r') as f:
                    # Read proteins, strip whitespace, convert to uppercase, and filter out empty lines
                    proteins_to_test = {line.strip().upper() for line in f if line.strip()}
            
                if not proteins_to_test:
                    print("  WARN: Protein filter file is empty. No additional filtering applied.")
                else:
                    print(f"  Loaded {len(proteins_to_test)} unique proteins from the filter file.")
                    count_before_user_filter = len(df_eval_raw)
                    # Filter the DataFrame to only include proteins from the user's list
                    df_eval_raw = df_eval_raw[df_eval_raw['protein'].str.upper().isin(proteins_to_test)]
                    count_after_user_filter = len(df_eval_raw)
                    print(f"  Filtered from {count_before_user_filter} to {count_after_user_filter} records based on the provided protein list.")

            except FileNotFoundError:
                print(f"  ERROR: Protein filter file not found at '{args.protein_filter_file}'. Exiting.", file=sys.stderr)
                sys.exit(1)
            except Exception as e:
                print(f"  ERROR: Failed to read or process protein filter file '{args.protein_filter_file}': {e}", file=sys.stderr)
                sys.exit(1)

        # --- Post-filtering summary ---
        final_filtered_count = len(df_eval_raw)
        unique_proteins = df_eval_raw['protein'].nunique()
        print(f"Proceeding with {final_filtered_count} records for evaluation.")
        print(f"These records correspond to {unique_proteins} unique proteins.")
    ...
    print("\n--- Pre-computing Strategies for Novel Proteins ---")
    protein_strategy_cache = {'deepsea': {}, 'sei': {}}
    all_unique_proteins_uc = {p['protein_name'].upper() for p in eval_inputs}
    
    for model_type in models_to_run_arg:
        if model_type in ['deepsea', 'sei']:
            model_resources = MODEL_SPECIFIC_RESOURCES_FOR_UNKNOWN_HANDLING.get(model_type)
            if not model_resources:
                print(f"  Skipping pre-computation for {model_type}: resources not loaded.")
                continue

            known_prots_for_model = set(model_resources["known_protein_ids_for_model"])
            novel_prots_for_model = sorted(list(all_unique_proteins_uc - known_prots_for_model))
            
            if novel_prots_for_model:
                print(f"Found {len(novel_prots_for_model)} unique novel proteins for model '{model_type}'.")
                strategies = batch_determine_protein_strategies(
                    protein_names_uc=novel_prots_for_model,
                    model_type=model_type,
                    model_specific_resources=model_resources,
                    global_protein_sequences_map_arg=GLOBAL_PROTEIN_SEQUENCES_MAP,
                    protein_id_to_tf_family_map_shared_arg=PROTEIN_ID_TO_TF_FAMILY_MAP_SHARED,
                    disabled_fallback_strategies_cfg=disabled_strategies_list # Pass the parsed list
                )
                protein_strategy_cache[model_type] = strategies
                print(f"Finished pre-computing for {model_type}.")
            else:
                print(f"No novel proteins found for model '{model_type}'.")
    ...
    workflow.add_node("filter_string_interactors", partial(filter_string_interactors_node, cisbp_tf_info_df=cisbp_tf_info_df, cisbp_pwm_dir=cisbp_pwm_dir, enable_two_hop=args.enable_two_hop_indirect_search))
    ...
    deepsea_pred_partial = partial(
        ...
        deepsea_protein_strategy_cache=protein_strategy_cache.get('deepsea', {})
    )
    ...
    sei_pred_partial = partial(
        ...
        sei_protein_strategy_cache=protein_strategy_cache.get('sei', {})
    )
    ...
    workflow.add_node("direct_prediction_node", direct_prediction_node)    
    ...
    workflow.add_conditional_edges(
        "generate_llm_prompt", # Source node is now the one that sets the style
        should_call_llm_or_predict_directly,
        {
            "call_llm": "get_llm_pred",
            "predict_directly": "direct_prediction_node",
            "error_path": "handle_error"
        }
    )
    ...
    if not eval_inputs: print("No samples to process due to limit=0.")
        ...
        for i, input_data in enumerate(eval_inputs):
            ...
            simple_output_path = os.path.join(output_dir_for_run, f"{run_identifier}_{safe_protein_name}{llm_name_part_for_filename}_simple.json")
            comp_output_path = os.path.join(output_dir_for_run, f"{run_identifier}_{safe_protein_name}{llm_name_part_for_filename}_comp.json")
            ...
            try:
                ...
                # --- Create Indirect Motif Hits Summary ---
                indirect_hits_summary = {}
                indirect_analysis_summary = final_state.get("indirect_motif_analysis_summary")
                if indirect_analysis_summary and indirect_analysis_summary.get("any_indirect_motif_found"):
                    all_hits = indirect_analysis_summary.get("interacting_tf_hits", [])
                    tfs_with_hits = [
                        hit.get('interacting_tf', 'UnknownTF') for hit in all_hits
                    ]
                    # Count hits per TF using a dictionary comprehension for clarity
                    indirect_hits_summary = {tf: tfs_with_hits.count(tf) for tf in set(tfs_with_hits)}
                    
                simple_output = {
                    "sample_id": f"{i+1}/{len(eval_inputs)}", # <<< 1. ADDED SAMPLE NUMBER
                    ...
                    # <<< 2. ADDED INDIRECT MOTIF SUMMARY
                    "indirect_motif_hits_summary": indirect_hits_summary if indirect_hits_summary else "No indirect hits found",                    
                ...
                try:
                    # Prune the state before making it serializable
                    pruned_state = prune_final_state_for_logging(final_state)                    
                    serializable_state = make_serializable(pruned_state)
    ...
    agg_filename_base = f"{llm_name_prefix}prompt_{args.prompt_style}"
